...
function snap(r){
  ...
  return {code:r.code,mode:r.mode,solo:r.mode==='solo',roomName:r.roomName||'',running:r.running,started:r.started,track:r.track,trackName:t.name,
    players:[...r.players.values()].map(p=>{const q=posFor(p,t);return{
      ...
      alive:p.alive,trail:p.trail.slice(-80),kills:p.kills,color:p.color,lap:p.lap,finish:p.finish,progress:p.progress,lane:p.lane,drift:p.drift,driftLevel:p.driftLevel,itemReady:(p.energy>=30&&Date.now()-p.lastSab>8000)
    }})};
}
function start(r){
  if(r.running)return;
...
io.on('connection',s=>{
  s.on('room:create',({nickname,ceo,key,track,mode,roomName,password,characterId}={})=>{
    ...
    const p=spawn(0);Object.assign(p,{id:s.id,nickname,bot:false,color:colors[0],characterId:Math.max(1,Math.min(8,Number(characterId)||1))});r.players.set(s.id,p);
    if(mode==='solo'){
      for(let i=1;i<MAX;i++){
        const b=spawn(i);
        Object.assign(b,{id:`bot-${crypto.randomUUID()}`,nickname:BOT_NAMES[i-1],bot:true,botSkill:BOT_SKILLS[i-1],characterId:i+1,color:colors[i%colors.length]});
  ...
  s.on('room:join',({code,nickname,password,characterId}={})=>{
    ...
    const p=spawn(r.players.size);Object.assign(p,{id:s.id,nickname:cleanNick(nickname)||'Piloto',bot:false,color:colors[r.players.size],characterId:Math.max(1,Math.min(8,Number(characterId)||1))});
